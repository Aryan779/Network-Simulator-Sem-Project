...
class Switch:
    ...
    def transmit(self):
        ...
        if dest_device_mac in mac_addr_list:
            ...
            devices=list(self.mac_table.keys())
            mac_addr_list=[device.mac_address for device in devices ]
            source_device=devices[mac_addr_list.index(self.frames[0][1])]
            dest_device=devices[mac_addr_list.index(dest_device_mac)]
            time.sleep(1)
            port = self.mac_table[dest_device]
            print(f"Frame sent from port {port}")
            if(self.opt1==1):
                res=self.send_frame_stop_and_wait(self.frames,dest_device)
            elif self.opt1==2:
                res=self.send_frame_gbn(self.frames,dest_device)
...
print("\n\n Used 1 to 100 mac_address for end_device & 101 to 150 for hub & 151 to 200 for bridge & & 201 to 250 for switch \n\n")
